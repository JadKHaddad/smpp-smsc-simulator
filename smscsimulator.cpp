...
private:
    ...
    {
        if (mq.size() == 0)
            return false; // no time-lists

        uint64_t firstListTime = (*mq.begin()).first;
        uint64_t now = currentUSecsSinceEpoch();
        if (firstListTime > now)
            return false; // nothing due to be deliverered
...
#define CMDEXP(A) CmdID::A, #A
    ...
    const char *cmdString(uint64_t id)
    {
        for (int i = 0; i < 34; i++)
        {
            if (cmdStrings[i].cmdid == id)
                return cmdStrings[i].name;
        }
        return "";
    }
...
private:
    ...
    {
        if (socket->bytes_to_read() < len)
            throw SMPPException();
        for (int i = 0; i < len; i++)
        {
            if (socket->recvA(mem[i]) == 0)
                throw SMPPException();
        }
        return 0;
    }
...
public:
    ...
    ~SMPPConnection()
    {
        if (command_received_body.body)
            delete[] command_received_body.body;
    ...
    bool get()
    {
        ...
        if (command_received_header.valid != PV_ALL)
        {
            ...
            catch (SMPPException e)
            {
                // no data
                if (debug)
                    std::cout << "HEADER No data - ABORT!" << std::endl;
                return false;
            }
        }
        ...
        if (body_length > 0)
        {
            ...
            {
                if (debug)
                    std::cout << "BODY No data" << std::endl;
                return false;
            }
    ...
    bool timedCheck(void)
    {
        ...
        if ((system_id.length() > 0) && ((bindState == SMPPSession::BindState::BS_TRX) || (bindState == SMPPSession::BindState::BS_RX)))
        {
            ...
            while (MessageDeliverer::getInstance_get(system_id, msg))
            {
                ...
                if (strstr(msg.destination_addr, system_id.c_str()) != NULL)
                { // system_id is in destination address - assume MO for testing purposes
            ...
            if ((cmdid == SMPP::CmdID::BindTransceiver) || (cmdid == SMPP::CmdID::BindReceiver) || (cmdid == SMPP::CmdID::BindTransmitter))
            {
                ...
                else
                {
                    ...
                    if ((esme_smpp_ver < 0x34) && (cmdid == SMPP::CmdID::BindTransceiver))
                        allowBind = false; // PROTOCOL transceiver bind only allowed for v3.4+
...
int main(int argc, const char *argv[])
{
    ...
    if (listensockfdAdmin > max_sd)
        max_sd = listensockfdAdmin;
